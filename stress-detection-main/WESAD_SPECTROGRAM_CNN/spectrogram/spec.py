...
def stft(X, fftsize=512, step=65, mean_normalize=True, real=False,
...
def spectro_pretty(data_input, nfft = 2048):
    ### Parameters ###
    fft_size = nfft # window size for the FFT
    step_size = fft_size/8 # distance to slide along the window (in time)
    spec_thresh = 4 # threshold for spectrograms (lower filters out more noise)
    
    # Only use a short clip for our demo
    wav_spectrogram = pretty_spectrogram(data_input.astype('float64'), 
                                         fft_size = fft_size, 
                                         step_size = step_size, 
                                         log = True, 
                                         thresh = spec_thresh)
